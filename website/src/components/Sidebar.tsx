...
export default function Sidebar() {
  ...
  return (
    <div
      ref={ref}
      className="h-full z-10 fixed ${isSidebarOpen ? 'open' : 'closed'}"
    >
      <button
        className={`${
          open ? 'hidden' : 'block'
        } absolute md:hidden p-2 bg-accent -rotate-90 rounded-b-lg mt-4`}
      ...
      <div
        className={`${
          open ? 'fixed' : 'hidden'
        } md:fixed w-60 md:w-72 md:flex h-full bg-sidebar flex-col pt-2 px-4 overflow-y-auto select-none`}
      >
        <button
        ...
        >
          {
            <DropdownIcon
              className={`${showSettings ? 'rotate-0' : 'rotate-[270deg]'}`}
            />
          }
          <span>Settings</span>
        </button>
        ...
        >
          {
            <DropdownIcon
              className={`${showClasses ? 'rotate-0' : 'rotate-[270deg]'}`}
            />
          }
          <span>Classes</span>
        </button>
            ...
            {classes.map((CLASS, index: number) => {
              return (
                <li
                  ...
                  id={`sidebar-${CLASS.name}`}
                  key={index}
                >
                  <Link
                    className='w-full border-l-4 border-transparent hover:border-accent px-2'
                    href={`/docs/classes/${CLASS.name}`}
                  >
                    {CLASS.name}
                  </Link>
                </li>
        ...
        >
          {
            <DropdownIcon
              className={`${showInterfaces ? 'rotate-0' : 'rotate-[270deg]'}`}
            />
          }
            ...
            {interfaces.map((INTERFACE, index: number) => {
              return (
                <li className='w-full p-1' key={index}>
                  <Link
                    className='w-full border-l-4 border-transparent hover:border-accent px-2'
                    href={`/docs/interfaces/${INTERFACE.name}`}
                  >
                    {INTERFACE.name}
                  </Link>
                </li>
