...
export default function Main({ name }: { name?: string }) {
  ...
  let constructor = result?.children?.find(
        ...
        switch (param.type.type) {
          case 'reference':
            constructorParams.push({
              name: param.name,
              type: symbolMap[param.type.target as number].sourceFileName
                .split('/')
                .pop()
                ?.replace(/\.[^.]+$/, ''),
              description: param.comment ? param.comment.summary![0]?.text : '',
              href: `/docs/${
                docs.children.find(
                  (child) =>
                    child.name ==
                    symbolMap[param.type.target as number].sourceFileName
                      .split('/')
                      .pop()
                      ?.replace(/\.[^.]+$/, '')
                )!.kind == 128
          ...
          case 'typeOperator':
            constructorParams.push({
              name: param.name,
              type: `${param.type.operator} ${symbolMap[
                (param.type.target as { target: number }).target
              ].sourceFileName
                .split('/')
                .pop()
                ?.replace(/\.[^.]+$/, '')}`,
              description: '',
              href: `/docs/interfaces/${symbolMap[
                (param.type.target as { target: number }).target
              ].sourceFileName
                .split('/')
                .pop()
          ...
          case 'intrinsic':
            constructorParams.push({
              ...
              href:
                param.type.name == 'any'
                  ? `https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#any&_blank`
          ...
          case 'literal':
            constructorParams.push({
              ...
              description: param.comment.summary
                ...
                : '',
              href: `https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/string&_blank`,
            })
            break
        }
      })
        ...
        return child.type.types.map((type) => {
          switch (type.type) {
            case 'intrinsic':
              return type.name
            case 'reference':
              return symbolMap[type.target]?.sourceFileName
                .split('/')
                .pop()!
                .replace(/\.[^.]+$/, '')
            case 'array':
              return `${symbolMap[type.elementType.target]?.sourceFileName
                .split('/')
                .pop()
                ?.replace(/\.[^.]+$/, '')}`
          }
  ...
  ;(result?.children as unknown as Child[]).forEach((child) => {
    ...
    href = child.sources[0].url.replace(
      /github\.com\/solacejs\/solace.js\/blob\/[a-f0-9]+/,
      'github.com/solacejs/solace.js/tree/main'
    )
    switch (child.kind) {
      case 1024:
      ...
      case 2048:
        ...
        child.signatures[0].parameters?.forEach((param: ParametersEntity) => {
          switch (param.type.type) {
            case 'reference':
              ...
              parameters.push({
                name: symbolMap[param.type.target as number].sourceFileName
                  .split('/')
                  .pop()!
                  .replace(/\.[^.]+$/, ''),
              })
            ...
            case 'array':
              switch (param.type.elementType.type) {
                case 'reference':
                  parameters.push({
                    name:
                      symbolMap[param.type.elementType.target].sourceFileName
                        .split('/')
                        .pop()
                        ?.replace(/\.[^.]+$/, '') + '[]',
                  })
        ...
        methods.push({
          name: child.name,
          href: href,
          description: child.signatures[0].comment
    ...
    <main className='w-full h-full flex'>
      <div className='z-0 w-full md:w-[calc(100%-18rem)] md:ml-72 h-full py-4 px-8'>
        <div className='w-full h-full'>
          ...
          <p>
            {
              result.comment?.summary.find((summary) => summary.kind == 'text')
                ?.text
            }
          </p>
          {signatures ? (
            signatures[0].comment && (
              <div>
                ...
                <CodeBlock language='ts'>
                  {`new ${result.name}(${constructorParams.map((param, index) =>
                    index == 0 ? param.name : ` ${param.name}`
                  )});`}
                </CodeBlock>
                  ...
                  <tbody>
                    {constructorParams.map((param, index) => {
                      return (
                        <tr
                          className={
                            index > 0 ? 'border-t border-t-nav-border' : ''
                          }
                          key={index}
                        >
                          ...
                          <td className='!pl-2.5 !py-5'>
                            <Link
                              className='link'
                              target={
                                param.href.endsWith('&_blank') ? '_blank' : ''
                              }
                              href={param.href.replace('&_blank', '')}
                            >
                              {param.type}
                            </Link>
                          </td>
          ...
          <div className='grid sm:grid-cols-1 mb-10 md:grid-cols-2 mt-3'>
            {properties.length > 0 && (
              <div>
                <button
                  ...
                  className='py-2 text-md font-bold uppercase flex gap-1 items-center'
                >
                  <DropdownIcon
                    className={showProperties ? 'rotate-0' : 'rotate-[270deg]'}
                  />
                  ...
                  <ul className='p-0 !mt-0 pl-2'>
                    {properties.map((property, index) => {
                      return (
                        <li
                          key={index}
                          className='mb-1 border-l-4 pl-2 border-l-transparent hover:border-accent'
                        >
                          <Link className='link' href={`#${property.name}`}>
                            {property.name}
                          </Link>
                        </li>
                      )
            ...
            {methods.length > 0 && (
              <div>
                ...
                >
                  <DropdownIcon
                    className={showMethods ? 'rotate-0' : 'rotate-[270deg]'}
                  />
                  <span>Methods</span>
                </button>
                {showMethods && (
                    ...
                    {methods.map((method, index) => {
                      return (
                        <li
                          ...
                          className='mb-1 border-l-4 pl-2 border-l-transparent hover:border-accent'
                        >
                          <Link className='link' href={`#${method.name}`}>
                            {method.name}
                          </Link>
                        </li>
                      )
          ...
          {properties.length > 0 && (
            <div>
              ...
              {properties.map((property, index) => (
                <div
                  className='my-4 mx-8 bg-code px-2 py-4 rounded-md'
                  id={property.name}
                  key={index}
                >
                  <div className='flex items-center justify-between'>
                    ...
                    {property.href && (
                      <Link
                        href={property.href}
                        target='_blank'
                        className='float-right mr-4 text-[var(--syntax-string)]'
                      >
                        <DevelopmentIcon />
                      </Link>
                    )}
                  ...
                  <div className='grid pl-2.5'>
                    ...
                    <div className='font-semibold mt-3'>Type:</div>
                  </div>
                </div>
              ))}
            </div>
